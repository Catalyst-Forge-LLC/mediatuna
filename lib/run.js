...
export async function runConversion({
    ...
    function encodeWithFfmpeg(args, { activeFileBar, knownDuration } = {}) {
        return runFfmpeg(args, {
            setActiveProc: progress.setActiveProc,
            onProgress: (chunk) => {
    ...
    async function runEncodePass({
        ...
        if (jobStatus === 'skip (normalized)' || jobStatus === 'skip (exists)') {
            stats.skipped++;
            logger.logFile(`Skipped${jobStatus.includes('normalized') ? ' (normalized)' : ''}: ${passName}`);
            return true;
        }
        ...
        const activeFileBar = progress.getOrCreateFileBar(barTotal, passName, {
    ...
    async function processFile(entry, index) {
        ...
        if (dryRun) {
            if (isSkippableStatus(videoStatus)) stats.skipped++;
            else if (videoStatus.startsWith('convert')) stats.done++;
            if (extractStatus) {
                if (isSkippableStatus(extractStatus)) stats.skipped++;
    ...
    for (let i = 0; i < preflight.length; i++) {
        if (progress.isShuttingDown()) break;
