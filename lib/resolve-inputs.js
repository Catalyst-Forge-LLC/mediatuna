...
export async function loadInputFiles(resolved, mediaMode, { stampDates = false } = {}) {
    if (resolved.files) return resolved.files;
    return discoverFiles(resolved.targetPath, resolved.recursive, mediaMode, { stampDates });
}
...
export function buildModeParts({
    ...
    deleteOriginals, dryRun, resume, jobs = 1, dupeReport = false, dupeHash = false,
    recupMap = false,
}) {
    if (recupMap) {
