...
When work starts, move this file to [partial/](./partial/). When shipped, fold IDs into [improvements.md](./improvements.md) and delete this file.
...
| `--recup-map --cleanup-originals` | `stat.size` of recup file vs file in `proposed-tree/` | **No** | **Yes** |
...
`--hash` is the only content-exact check. It is **opt-in**, **dupe-report only**, and does **not** run for recup place/apply/cleanup.
...
|----|----------|-------|-------|--------|
| HS-01 | P0 | **Deletes bypass Recycle Bin** | `fs.unlinkSync` | Prefer OS trash (`--trash`, default for deletes). Keep `--delete-permanent` (or type `DELETE`) for true unlink. |
| HS-02 | P0 | **Recup cleanup is size-only** | Same `stat.size` ⇒ eligible to delete recup source | Require **SHA-256 match** (or `--hash`) before recup delete. Size-only is report-only. |
| HS-03 | P0 | **`--force` overwrites outputs** | Existing MP4/MP3 replaced | Confirm when TTY and count > 1; always log paths. Never combine `--force` with delete in one argv (already partly restricted). |
| HS-04 | P0 | **`--stamp-dates` renames in place** | Sources renamed; `--backup` optional | Warn if `--backup` omitted and count > N; dry-run default hint in help. |
| HS-05 | P1 | **In-place convert is the default** | Writes next to sources in **cwd** | Startup line: “writing N files beside sources in \<dir\>.” Optional `--require-output` / config for nervous archives. |
| HS-06 | P1 | **`--recursive` on the wrong tree** | Encodes (and later deletes) everything under the path | If discovered count > threshold (e.g. 50), print count + root and require Enter / `--yes` when TTY. |
| HS-07 | P1 | **No free-space preflight** | Can fill the disk mid-batch | Estimate output from source sizes × factor; abort or warn if destination volume is short. |
| HS-08 | P2 | **Failed output removed by default** | Useful after a crash if you wanted the stub | Keep default; document. Optional `--keep-partial` already exists. |

### 3.2 Wrong-file / false “same”

| ID | Severity | Issue | Today | Target |
|----|----------|-------|-------|--------|
| HS-10 | P0 | **Recup `--apply` copies from size+path score** | Wrong gold copy ⇒ wrong folder | `--apply` refuses ambiguous rows (already). Add optional `--hash` to confirm the scored copy is byte-identical before copy. Mark size-only placements in the report. |
...
| HS-12 | P1 | **`skip (exists)` is path-only** | Any file at `clip.mp4` skips, even empty/wrong | If size is 0 or probe fails, do not skip (treat as convert / fail). |
| HS-13 | P2 | **Name+size dupes look like certainty** | Report heading is easy to over-read | Label sections: “same name and size (not hashed)” vs “SHA-256 match.” |

### 3.3 Quality / one-way loss

| ID | Severity | Issue | Today | Target |
|----|----------|-------|-------|--------|
| HS-20 | P1 | **Lossy encode is silent except FLAC-style warn** | AVI→H.264 / WMA→MP3 is the product | One preflight banner: “outputs are lossy; keep sources until you have checked playback.” |
...
| HS-22 | P1 | **`--no-verify` can mark a bad file “done”** | Resume/delete blocked with `--no-verify`; convert still writes | Keep delete blocked. Log a loud warning that outputs are unchecked. |
...
| HS-32 | P2 | **`--jobs` fills disk and heat faster** | User picks N | Preflight: “N concurrent encodes.” Pair with HS-07. |
...
| HS-44 | **Trash instead of unlink** | Desktop converters, File Explorer | See HS-01 | High |
| HS-45 | **Free-space check** | HandBrake, Shutter | See HS-07 | High |
| HS-46 | **Hash-confirm before recup apply/cleanup** | Recovery tools that compare copies | See HS-02, HS-10 | High |
| HS-47 | **Large-batch confirm** | Installers, some CLIs (`-y`) | See HS-06 | Medium |
...
| Case | Type |
|------|------|
| Two different files, same size + ext → recup cleanup must **not** delete without hash | Unit + fixture |
| SHA-256 match → cleanup allowed | Unit |
| Empty existing `.mp4` is not `skip (exists)` | Unit |
| `--dry-run --cleanup-originals` never unlinks | Integration |
| Disk-space warn when estimate > free (mock `statfs`) | Unit |
| `--sample 5` writes a short output and does not replace the full source | Integration |
| Trash API vs unlink: `--delete-originals` default path | Integration (Windows) |
