/**
 * Compile handbook markdown into a static shell at site/docs/dist/.
 * Sources: site/docs/<id>.md, else repo docs/<id>.md.
...
const handbookRoot = join(root, '..', '..', 'docs');
const outDir = join(docsRoot, 'dist');
const nav = JSON.parse(readFileSync(join(docsRoot, '_nav.json'), 'utf8'));
const githubBlob = 'https://github.com/Catalyst-Forge-LLC/mediatuna/blob/main';

function resolveDocSource(id) {
	const local = join(docsRoot, `${id}.md`);
	if (existsSync(local)) return local;
	const handbook = join(handbookRoot, `${id}.md`);
	if (existsSync(handbook)) return handbook;
	return null;
}

function rewriteHandbookLinks(md) {
	return md
...
<head>
	...
	<title>${escapeHtml(title)} · MediaTuna docs</title>
	<meta name="description" content="MediaTuna documentation — ${escapeHtml(title)}" />
...
<body>
	<header class="docs-top">
		...
		<a class="docs-brand" href="/docs/">MediaTuna <span>docs</span></a>
		<nav class="docs-top-links">
			<a href="/">Home</a>
			<a href="/install">Install</a>
			<a href="https://github.com/Catalyst-Forge-LLC/mediatuna">GitHub</a>
		</nav>
	</header>
...
function main() {
	...
	for (let idx = 0; idx < items.length; idx += 1) {
		...
		const src = resolveDocSource(item.id);
		if (!src) {
