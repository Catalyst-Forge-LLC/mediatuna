...
export function classifyHits(inputPath, nameHits, sizeHits) {
    ...
    const sizeOnly = sizeHits.filter(hit => !known.some(item => sameResolvedPath(item, hit.path)));
...
export function formatDupeReportLines({
    ...
    section('Size only (name differs — possible renamed copy)', withSize, row => row.sizeOnly);
...
export async function buildDupeRows(files, {
    ...
    for (const input of files) {
        ...
        try {
            ...
            let sizeHits = searchFn(buildSizeQuery(size));
            ...
            if (hashFn && classified.sizeOnly.length > 0) {
                if (useEverythingHash) {
                    ...
                    const hashHits = searchFn(buildSizeHashQuery(size, digest));
