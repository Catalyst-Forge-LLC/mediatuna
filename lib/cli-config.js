...
export function buildCliConfig(values, positionals, { cwd = process.cwd(), homedir = os.homedir() } = {}) {
    ...
    if (values.hash && !values['dupe-report']) {
        throw new CliConfigError('--hash is only valid with --dupe-report.');
    }
    ...
    if (values['recup-map'] && values['delete-originals']) {
        throw new CliConfigError('--recup-map cannot be used with --delete-originals.');
    }

    if (values['recup-map'] && values['cleanup-originals']) {
        throw new CliConfigError('--recup-map cannot be used with --cleanup-originals.');
    }
