...
export const STAMP_PREFIX_RE = /^(\d{4}-\d{2}-\d{2}(?:_\d{6}Z)?)_/;
...
export function formatStampPrefix(parsed) {
    if (!parsed) return null;
    const date = `${parsed.year}-${parsed.month}-${parsed.day}`;
    if (!parsed.hasTime) return date;
    return `${date}_${parsed.hour}${parsed.minute}${parsed.second}Z`;
}
...
export function stampedOutputStem(basename, meta, { preferMtime = false } = {}) {
    ...
    const prefix = formatStampPrefix(resolved.parsed);
...
export function resolveStampDate(meta, { preferMtime = false } = {}) {
    ...
    if (preferMtime && meta?.modified_time) {
        const fromMtime = parseCreationDate(meta.modified_time) || (
            !Number.isNaN(new Date(meta.modified_time).getTime())
                ? fromUtcDate(new Date(meta.modified_time), true)
                : null
        );
        if (fromMtime) {
            return { parsed: fromMtime, source: 'mtime', raw: meta.modified_time };
        }
    }
...
export function planStampRenames(files, {
    ...
    for (const input of files) {
        ...
        const stamp = formatStampPrefix(resolved.parsed);
