...
export function buildNameSizeQuery(basename, size) {
    return `size:${size} nopath:exact:${quoteEverythingTerm(basename)}`;
}

export function buildSizeQuery(size) {
    return `size:${size}`;
}

export function buildSizeHashQuery(size, sha256) {
    return `size:${size} sha256:${String(sha256).toLowerCase()}`;
}
